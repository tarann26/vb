...
*Overturned 2026-08-18 (see `.superpowers/sdd/2026-08-18-phase-5c-server-rendered-seo/hostname-move-brief.md`): the owner decided to purge `vb.aionxxxi.uk` from Cloudflare entirely, with no redirect, which would leave every canonical pointing at a host that no longer resolves. `seo.url` moved to `https://viabiancarestaurant.com` ahead of that purge. Left deliberately unmoved is the second half of D6 below — the Worker still reads the canonical from `seo.url`, not from request origin — so this was a value change, not a reversal of the mechanism.*
