...
The following were found by the whole-branch review that closed out this plan (final review round) and deliberately left for Plan 4 rather than built here:

- **PDF replacement has no endpoint.** The spec lists it as a dashboard capability, but the menus live at `public/menus/*.pdf`, `commitFiles`' allowlist permits only `src/content/*.json` and `assets-source/<cat>/<file>`, and `POST /api/upload` rejects PDFs at format detection. Plan 4 will build the UI and find nothing to call.
- **Upload returns `assets-source/food/<hash>.jpg`, not the path content JSON needs.** The derivative `npm run images` actually generates is `/food/<hash>.webp`. Plan 4 must derive it; getting it wrong is a broken image *and* a failed deploy (the content guards this plan built catch a dangling image reference at `npm run test:deploy` time, not silently).
- **Batch photo uploads into one commit.** One commit per upload means Cloudflare cancels superseded builds, which now correctly report `failed` (Task 8) -- so uploading 12 photos for a future phase shows most of them "failed" in the dashboard despite every one landing on `main`. `commitFiles` already takes N files; Plan 4 just needs to call it with more than one.
- **`convertHeic` is unwired and its bundle guard is inert.** `src/test/bundle.test.ts`'s HEIC-out-of-bundle check is `skipIf(!existsSync('dist/assets'))`, and `npm run test:deploy` runs before `npm run build` in the documented build command -- so it is the one skipped test on every run today. Nothing outside `src/admin/` imports the module yet, so the check currently passes either way, for the wrong reason. Plan 4 is the first real caller: keep the dynamic import `convertHeic` already uses, because a `dist/` grep alone will not catch a regression here -- the import-graph test is the one that actually does.
- **Owner-facing error wrapping is not built.** GitHub failures currently surface as developer sentences (e.g. `"GitHub returned 500 -- upstream boom"`), a publish conflict (someone else published first) returns `502` where `409` would let the dashboard branch on status instead of parsing a message, and `GET /api/build-status` gives a commit link and a preview URL but no build log -- so a failed build tells her it failed and gives her nothing to act on. Plan 4 owns translating these into sentences she can use.
