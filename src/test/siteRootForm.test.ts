...
//       applies it to plan()'s OUTPUT, which holds image-host URLs whether or
//       not the rewrite has been run, so the undo is exercised there today.
...
describe('siteRootForm', () => {
  it('turns a url on the image host back into the path it was written from', () => {
    expect(siteRootForm(`"image": "${HOST}/food/pizza1.webp"`)).toBe('"image": "/food/pizza1.webp"');
  });

  it('decodes the percent-encoding a spaced filename picked up on the way out', () => {
    expect(siteRootForm(`"${HOST}/mocktails/signor%20bianca.webp"`)).toBe('"/mocktails/signor bianca.webp"');
  });

  it('undoes a css url() and a jsx attribute in the same text', () => {
    const text = `a { background-image: url("${HOST}/hero/brick.webp"); }\n<img src="${HOST}/food/x.webp" />`;
    expect(siteRootForm(text)).toBe('a { background-image: url("/hero/brick.webp"); }\n<img src="/food/x.webp" />');
  });

  it('undoes every occurrence, not only the first', () => {
    expect(siteRootForm(`${HOST}/a.webp ${HOST}/b.webp ${HOST}/c.webp`)).toBe('/a.webp /b.webp /c.webp');
  });
  ...
  // The reason this is safe to read an assertion through. A photograph on
  // somebody else's host is not touched, so an assertion that used to fail on
  // one still fails on one.
  it('leaves a url on another host alone, including one that starts the same way', () => {
    const text = `"https://evil.example/food/x.webp" "${HOST}.evil.example/food/x.webp"`;
    expect(siteRootForm(text)).toBe(text);
  });

  it('leaves prose that merely names the host alone', () => {
    expect(siteRootForm(`the bucket is served from ${HOST} today`)).toBe(
      `the bucket is served from ${HOST} today`,
    );
  });
});
