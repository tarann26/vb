...
// five out of COPY_FIELDS entirely -- see below. Dipped to 25 for one day,
// 2026-08-17 to 2026-08-18, while `hero.reserveButton`'s in-place affordance
// was excluded rather than the field itself -- see below for why that
// exclusion is gone again.)
//
...
// redirect to /blog. That was right as far as it went, and it left five
// controls on /edit/manage that changed nothing anywhere -- because this
// list only ever governed the LIVE affordance, never the dashboard. The
// review fix took those five out of COPY_FIELDS itself, so there is nothing
// left here to exclude: no descriptor, no dashboard control, no in-place
// affordance. blogsPage.heading and blogsPage.intro were never part of it
// and are untouched -- BlogIndex.tsx paints both through renderText.
//
// A sixth exclusion lived here even more briefly than the fifth, for the
// opposite reason: not a control that turned out to change nothing, but a
// control the owner asked back. `hero.reserveButton` landed in the set
// below on 2026-08-17 when the Reserve a Table button's only render call
// site left Hero.tsx (that commit's own brief covers the whole cascade).
// The owner reversed the request the next day, the call site is back in
// Hero.tsx, and this list follows it back out -- see that entry's own
// history, now recorded where `press.readArticle` sits below.
...
const NOT_EDITABLE_IN_PLACE_COPY_FIELDS = new Set([
  ...
  'nav.pagesLabel',
  // `hero.reserveButton` sat here from 2026-08-17 to 2026-08-18, while the
  // Reserve a Table button had no render call site in Hero.tsx to be an
  // in-place affordance over. The owner reversed that request the next day;
  // the button is back in Hero.tsx, so this entry is not.
